...
function sumOfCodes(str) {
  // throw new Error('Not implemented');
  return str
    ? [...str].reduce((acc, _, index) => acc + str.charCodeAt(index), 0)
    : 0;
}
...
function countVowels(str) {
  ...
  return [...str].reduce(
    (acc, curr) => (vowels.includes(curr) ? acc + 1 : acc),
    0
  );
}
...
function isPalindrome(str) {
  // throw new Error('Not implemented');
  return (
    str.replace(/[, " ",?,!]/gm, '').toLowerCase() ===
    [...str.replace(/[, " ",?,!]/gm, '')].reverse().join('').toLowerCase()
  );
}
...
function extractNameFromTemplate(value) {
  // throw new Error('Not implemented');
  return value.split('Hello, ').pop().replace('!', '');
}
...
function encodeToRot13(str) {
  ...
  return str.replace(
    /[A-Za-z]/g,
    (currLetter) => alphabet[alphabet.indexOf(currLetter) + 13]
  );
}
